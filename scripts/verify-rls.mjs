...
console.log('\nNotitas')
const { data: note, error: noteErr } = await A.client
  .from('notes')
  .insert({ couple_id: couple.id, author_id: A.id, recipient_id: B.id, body: 'Te espero a las 8' })
  .select('id')
  .single()
ok(!noteErr, 'A leaves a note for B')
const { data: cNotes } = await C.client.from('notes').select('id')
ok(cNotes?.length === 0, 'C cannot see notes')
const { error: forgedNote } = await C.client.from('notes').insert({ couple_id: couple.id, author_id: A.id, recipient_id: B.id, body: 'x' })
ok(Boolean(forgedNote), 'C cannot leave notes in the couple')
const { error: noteToC } = await A.client.from('notes').insert({ couple_id: couple.id, author_id: A.id, recipient_id: C.id, body: 'x' })
ok(Boolean(noteToC), 'notes only go to the partner')
const { error: editNote } = await B.client.from('notes').update({ body: 'otra' }).eq('id', note.id)
const { data: stillSame } = await A.client.from('notes').select('body').eq('id', note.id).single()
ok(!editNote ? stillSame?.body === 'Te espero a las 8' : true, 'notes cannot be edited')
await C.client.from('notes').delete().eq('id', note.id)
const { data: survives } = await B.client.from('notes').select('id').eq('id', note.id)
ok(survives?.length === 1, 'C cannot delete it')
const { error: readNote } = await B.client.from('notes').delete().eq('id', note.id)
const { data: gone } = await A.client.from('notes').select('id').eq('id', note.id)
ok(!readNote && gone?.length === 0, 'B reads it: the note is deleted for everyone')
...
const { data: reply, error: replyErr } = await B.client
  ...
  .insert({ couple_id: couple.id, memory_id: memoryId, author_id: B.id, body: 'Qué bonito' })
  .select('id')
  .single()
ok(!replyErr, 'B replies to A’s memory')
const { data: cReplies } = await C.client.from('memory_comments').select('id')
ok(cReplies?.length === 0, 'C cannot read replies')
const { error: cReply } = await C.client.from('memory_comments').insert({ couple_id: couple.id, memory_id: memoryId, author_id: C.id, body: 'x' })
ok(Boolean(cReply), 'C cannot reply')
const { error: forgedReply } = await A.client.from('memory_comments').insert({ couple_id: couple.id, memory_id: memoryId, author_id: B.id, body: 'x' })
ok(Boolean(forgedReply), 'A cannot reply in B’s name')
await A.client.from('memory_comments').delete().eq('id', reply.id)
const { data: replyStays } = await B.client.from('memory_comments').select('id').eq('id', reply.id)
ok(replyStays?.length === 1, 'A cannot delete B’s reply')
const sub = { endpoint: `https://push.example/${run}`, p256dh: 'k', auth: 'a' }
const { error: subErr } = await A.client.from('push_subscriptions').insert({ ...sub, user_id: A.id })
ok(!subErr, 'A registers a device for notifications')
const { data: cSubs } = await C.client.from('push_subscriptions').select('id')
ok(cSubs?.length === 0, 'C cannot see anyone’s devices')
const { data: bSubs } = await B.client.from('push_subscriptions').select('id')
ok(bSubs?.length === 0, 'not even B sees A’s devices')
const { error: cSub } = await C.client.from('push_subscriptions').insert({ ...sub, endpoint: sub.endpoint + '/c', user_id: C.id })
ok(Boolean(cSub), 'outsiders cannot register devices')
