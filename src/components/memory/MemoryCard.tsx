      ...
      {cover ? (
        <div className="relative">
          ...
          {memory.photos.length > 1 && (
            <span className="absolute top-3 right-3 rounded-full bg-black/40 px-2.5 py-1 text-[12px] font-semibold text-white backdrop-blur-md tabular-nums">
...
export function MemoryRow({ memory }: { memory: Memory }) {
  ...
  return (
    ...
    >
      {cover ? (
        <SignedImage path={cover.thumbPath ?? cover.storagePath} frameClassName="size-[60px] shrink-0 rounded-[14px]" />
      ) : (
        <span className="grid size-[60px] shrink-0 place-items-center rounded-[14px] bg-surface font-serif text-[22px] text-accent">“</span>
      )}
      <div className="min-w-0 flex-1">
        <p className="truncate text-[16px] font-semibold">{memory.title || (cover ? 'Sin título' : memory.body?.split('\n')[0])}</p>
